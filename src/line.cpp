...
{
    float t = 0;
    if (!(numbers_comparison::is_approximately_equal(m_first_point.get_x(), m_second_point.get_x())))
    {
        t = (point.get_x() - m_second_point.get_x()) / (m_first_point.get_x() - m_second_point.get_x());
    }
    else
    {
        t = (point.get_y() - m_second_point.get_y()) / (m_first_point.get_y() - m_second_point.get_y());
    }
    return t;
}
float Line::length()
{
    return static_cast<float>(std::sqrt(std::pow(m_second_point.get_x() - m_first_point.get_x(), 2) + std::pow(m_second_point.get_y() - m_first_point.get_y(), 2)));
}
bool in_parametric_range(float parametric_view)
{
    return (parametric_view > 0) && (parametric_view < 1);
}
Point Line::make_vector(bool reversed) const
{
...
{
    Point first_row = make_vector(true);
    Point second_row = other_line.make_vector();
    std::array<std::array<float, 2>, 2> A {{{first_row.get_x(), second_row.get_x()}, {first_row.get_y(), second_row.get_y()}}};
    ...
    if ((!in_parametric_range(intersection_parameters.get_x()) || (!in_parametric_range(intersection_parameters.get_y()))))
    {
        is_intersects = false;
    }
    Point intersection_point = (coordinate_view(intersection_parameters.get_x()));
